    ...
    public void run()
    {
        try
        {
            ...
            while(true)
            {
                ...
                dt.Rew=new double[dt.Agent][dt.State];
                for(int i=0;i<dt.Agent;i++)
                {
                    ArrayList at=dt.reward[i];
                    for(int j=0;j<at.size();j++)
                    {
                        dt.Rew[i][j]=Double.parseDouble(at.get(j).toString());
                    }
                }
                
                for(int i=0;i<dt.Rew.length;i++)
                {
                    for(int j=0;j<dt.Rew[0].length;j++)
                        System.out.print(dt.Rew[i][j]+" ");
                    System.out.println();
                    
                }
                
                Regret rg=new Regret(count);
                rg.find();                
